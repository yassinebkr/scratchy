...
// Phase 32: User-to-User Messaging
...
// ── P3: Resilience layer ─────────────────────────────────────────────────────
// Exception rate limiter: too many uncaught errors in a short window = restart
// (prevents infinite corrupt-state loops while tolerating occasional blips)
const _exceptionLog = [];
...
process.on('uncaughtException', function(err) {
  ...
  const now = Date.now();
  _exceptionLog.push(now);
  // Trim old entries
  while (_exceptionLog.length && _exceptionLog[0] < now - EXCEPTION_WINDOW_MS) _exceptionLog.shift();
  if (_exceptionLog.length >= EXCEPTION_LIMIT) {
    console.error(`[Scratchy] ⛔ ${EXCEPTION_LIMIT} uncaught exceptions in ${EXCEPTION_WINDOW_MS/1000}s — restarting to recover`);
    process.exit(1); // systemd Restart=always will bring us back
  }
});
...
// ── Self-watchdog: event loop hang detection ─────────────────────────────────
// If the event loop blocks for >20s, something is catastrophically wrong.
// Log it and exit — systemd Restart=always brings us back clean.
let _lastTick = Date.now();
let _watchdogInterval = null;
function startWatchdog() {
  // Update _lastTick every 5s (proves event loop is alive)
  const tickInterval = setInterval(() => { _lastTick = Date.now(); }, 5_000);
  tickInterval.unref();

  // Check every 10s if the last tick is stale (indicates event loop freeze)
  _watchdogInterval = setInterval(() => {
    const staleness = Date.now() - _lastTick;
    if (staleness > 20_000) {
      console.error(`[Scratchy] ⛔ Event loop frozen for ${(staleness/1000).toFixed(0)}s — forcing restart`);
      process.exit(1);
    }
  }, 10_000);
  _watchdogInterval.unref();
  console.log('[Scratchy] 🐕 Self-watchdog active (20s hang detection)');
}

// ── Heap pressure monitor (every 60s) ────────────────────────────────────────
let _lastHeapWarnLevel = 0;
setInterval(() => {
  const heap = process.memoryUsage();
  const usedMB = heap.heapUsed / 1024 / 1024;
  const totalMB = heap.heapTotal / 1024 / 1024;
  const pct = (heap.heapUsed / heap.heapTotal) * 100;
  if (pct >= 90 && _lastHeapWarnLevel < 90) {
    console.error(`[Scratchy] ⚠️ HEAP CRITICAL: ${usedMB.toFixed(0)}MB / ${totalMB.toFixed(0)}MB (${pct.toFixed(0)}%)`);
    _lastHeapWarnLevel = 90;
  } else if (pct >= 80 && _lastHeapWarnLevel < 80) {
    console.warn(`[Scratchy] ⚠️ Heap pressure: ${usedMB.toFixed(0)}MB / ${totalMB.toFixed(0)}MB (${pct.toFixed(0)}%)`);
    _lastHeapWarnLevel = 80;
  } else if (pct < 75) {
    _lastHeapWarnLevel = 0; // Reset warnings when pressure drops
  }
}, 60_000).unref();
...
// ── Load JWT secret from systemd credentials if available ──
function loadJwtSecret() {
  const credsDir = process.env.CREDENTIALS_DIRECTORY;
  if (credsDir) {
    const credPath = path.join(credsDir, "jwt-secret");
    try {
      if (fs.existsSync(credPath)) {
        const secret = fs.readFileSync(credPath, "utf8").trim();
        if (secret) {
          console.log("[Scratchy] Loaded JWT secret from systemd credentials");
          return secret;
        }
      }
    } catch (err) {
      console.warn("[Scratchy] Failed to read credential:", err.message);
    }
  }
  return null;
}
...
// Load JWT secret from systemd credentials, or derive from auth token
const JWT_SECRET = loadJwtSecret() || crypto.createHmac("sha256", AUTH_TOKEN).update("scratchy-jwt").digest("hex");
...
try {
  ...
  usageAggregator.initialize().then(() => {
    ...
    // Also set global for AdminWidget dynamic access
    global.usageQuery = usageQuery;
...
function handleHistoryApi(req, res) {
  ...
  const beforeSeq = url.searchParams.get("before");
  ...
  const maxLimit = 200; // Prevent abuse
  const effectiveLimit = Math.min(limit, maxLimit);

  console.log(`[Scratchy] /api/history request — session=${sessionKey}, limit=${effectiveLimit}${beforeSeq ? ", before="+beforeSeq : ""}${afterSeq ? ", after="+afterSeq : ""}`);
  ...
  // Support before/after cursors for infinite scroll
  // Also support fromSeq/toSeq for gap fill (MessageStore gap detection)
  let filteredMessages = allMessages;
  let hasMore = false;
  let nextCursor = null;
  let prevCursor = null;
  
  // Filter by sequence range (fromSeq/toSeq for gap fill)
  if (fromSeq != null || toSeq != null) {
    const from = fromSeq != null ? parseInt(fromSeq, 10) : 0;
    const to = toSeq != null ? parseInt(toSeq, 10) : Infinity;
    filteredMessages = allMessages.filter((m, idx) => {
      // Use explicit seq if available, otherwise index
      const msgSeq = m.seq != null ? m.seq : idx;
      return msgSeq >= from && msgSeq <= to;
    });
  }
  
  // Apply before/after cursors for infinite scroll
  // before = load older messages (lower seq numbers)
  // after = load newer messages (higher seq numbers, for gap fill)
  if (beforeSeq != null) {
    const before = parseInt(beforeSeq, 10);
    // Get messages strictly before the cursor
    const beforeMessages = filteredMessages.filter((m, idx) => {
      const msgSeq = m.seq != null ? m.seq : idx;
      return msgSeq < before;
    });
    // Take the last N messages (oldest relative to cursor)
    if (beforeMessages.length > effectiveLimit) {
      hasMore = true;
      const startIdx = beforeMessages.length - effectiveLimit;
      filteredMessages = beforeMessages.slice(startIdx);
      // Cursor for next page (older than what we just returned)
      const oldestReturned = filteredMessages[0];
      if (oldestReturned) {
        nextCursor = oldestReturned.seq != null ? oldestReturned.seq : (beforeMessages.length - effectiveLimit);
      }
    } else {
      filteredMessages = beforeMessages;
    }
    prevCursor = before; // Can navigate forward with after=before
  } else if (afterSeq != null) {
    const after = parseInt(afterSeq, 10);
    // Get messages strictly after the cursor
    const afterMessages = filteredMessages.filter((m, idx) => {
      const msgSeq = m.seq != null ? m.seq : idx;
      return msgSeq > after;
    });
    // Take first N messages (newest relative to cursor)
    if (afterMessages.length > effectiveLimit) {
      hasMore = true;
      filteredMessages = afterMessages.slice(0, effectiveLimit);
      // Cursor for next page (newer than what we just returned)
      const newestReturned = filteredMessages[filteredMessages.length - 1];
      if (newestReturned) {
        nextCursor = newestReturned.seq != null ? newestReturned.seq : effectiveLimit;
      }
    } else {
      filteredMessages = afterMessages;
    }
    prevCursor = after; // Can navigate backward with before=after
  } else {
    // No cursor — initial load, return most recent messages
    if (filteredMessages.length > effectiveLimit) {
      hasMore = true;
      // Get last N messages (most recent)
      const startIdx = filteredMessages.length - effectiveLimit;
      filteredMessages = filteredMessages.slice(startIdx);
      // Cursor to load older messages
      const oldestReturned = filteredMessages[0];
      if (oldestReturned) {
        nextCursor = oldestReturned.seq != null ? oldestReturned.seq : startIdx;
      }
    }
  }
  ...
  console.log(`[Scratchy] /api/history response — ${filteredMessages.length}/${allMessages.length} messages (hasMore=${hasMore}), ${(responseJson.length/1024).toFixed(0)}KB`);
...
<body><div class="c"><div class="s">🔧</div><h2>Service Update in Progress</h2><p>A new version is being prepared. Please check back shortly.</p></div></body></html>`);
  ...
  if (ext === ".html" || relPath === "index.html") {
    ...
    // Versioned assets (CSS/JS with ?v=xxx) — cache but allow updates when version changes
      ...
      for (const part of parts) {
        if (part.name === "message") {
          ...
          if (isTextMime(mimeType)) {
            ...
            // Images: save to disk with predictable path, then reference in message.
            // This allows the agent's image tool to read the file directly.
            ...
            message = (message ? message + "\n\n" : "") +
              `📎 Attached image: ${label} (${sizeKB}KB, ${mimeBase})\nSaved to: ${uploadPath}\nPlease read and analyze this image.`;
            console.log(`[Scratchy] Saved image upload: ${uploadPath} (${sizeKB}KB, ${mimeBase})`);
          } else {
...
function handleWorkspaceFileApi(req, res, url) {
  ...
  // Normalize: strip leading slash if present (absolute paths are resolved relative to workspace)
...
const server = http.createServer((req, res) => {
  ...
  // ── Health endpoint (unauthenticated, for monitoring / systemd) ──
  if (url.pathname === "/health" || url.pathname === "/api/health") {
    const heap = process.memoryUsage();
    const uptime = process.uptime();
    const sessions = typeof wsSessions !== 'undefined' ? wsSessions.size : 0;
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({
      status: "ok",
      ...
      sessions,
      exceptionsLastMin: _exceptionLog.filter(t => t > Date.now() - 60000).length
    }));
    return;
  }
  ...
  if (url.pathname === "/api/version") {
    ...
    // Dynamic version: read from version store instead of cached BUILD_VERSION
      ...
      if (effectiveVersion) {
        ...
        if (vWebDir) {
          try {
            ...
            userHash = effectiveVersion; // Use version tag as hash
          } catch(e) {}
        }
      }
    } else if (!isAdmin) {
      // Fallback for non-admin without explicit version
      if (versionStore) {
        const versions = versionStore.list();
        const def = versions.find(v => v._isDefault) || versions.find(v => v.status === 'live');
        if (def) {
    ...
    // Admin falls through to BUILD_VERSION + BUILD_HASH
...
<script>localStorage.setItem("scratchy-post-auth-widget","${postAuthWidget}");setTimeout(function(){window.location.href="/";},1500);</script></body></html>`);
  ...
  // Phase 32: User-to-user messaging REST API
  else if (url.pathname === '/api/messages/users') {
    if (!messageRouter) { res.writeHead(503); res.end('Messaging not initialized'); return; }
    const userId = req._authResult?.user?.id || '_legacy';
    const user = req._authResult?.user;
    const isAdmin = user?.role === 'admin';

    // Sync all auth users to MessageRouter shadow table (admins should see everyone)
    if (isAdmin && authSystem?.userStore) {
      const allAuthUsers = authSystem.userStore.listUsers();
      for (const u of allAuthUsers) {
        messageRouter.registerUser(u.id, null, u.displayName || u.email, u.role === 'admin');
      }
    }

    const users = messageRouter.listUsers(userId);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (!messageRouter) { res.writeHead(503); res.end('Messaging not initialized'); return; }
    const userId = req._authResult?.user?.id || '_legacy';
    const conversations = messageRouter.getConversations(userId);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ conversations }));
    return;
  } else if (url.pathname === '/api/messages/send') {
    ...
    req.on('end', () => {
      try {
        const data = JSON.parse(body);
        const fromUser = req._authResult?.user?.id || '_legacy';
        const result = messageRouter.sendMessage(fromUser, data.toUser, data.content, data.contentType);
        res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    return;
  } else if (url.pathname === '/api/messages/history') {
    if (!messageRouter) { res.writeHead(503); res.end('Messaging not initialized'); return; }
    const userId = req._authResult?.user?.id || '_legacy';
    ...
    const convId = messageRouter._getConversationId(userId, otherUser);
    const messages = messageRouter.getMessages(convId, beforeId, limit);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ conversationId: convId, messages, otherUser }));
    return;
...
function _handleSmartProxy(clientWs, clientId, lastSeq, userInfo, deviceId, userAgent) {
  ...
  if (existing && existing.gatewayWs && existing.gatewayWs.readyState === WebSocketClient.OPEN) {
    ...
    if (existing._userInfo?.user) {
      ...
      // Phase 32: Update message router presence on reconnect
      if (messageRouter) {
        try {
          const user = existing._userInfo.user;
          messageRouter.registerUser(user.id, clientId, user.displayName || user.email, user.role === 'admin');
          messageRouter.updatePresence(user.id, true);
          console.log(`[Scratchy] User reconnected to message router: ${user.email} (${user.id})`);
        } catch(e) { console.error('[Scratchy] Message router reconnect failed:', e.message); }
      }
    }
  ...
  const gwWs = new WebSocketClient(`ws://127.0.0.1:${GATEWAY_PORT}/`, {
    headers: { Origin: "https://scratchy.clawos.fr" }
  });
  ...
  gwWs.on("message", (data) => {
    ...
    if (!session.handshakeComplete && frame.type === "res") {
      ...
      // Phase 32: Register user with message router for direct messaging
      if (messageRouter && session._userInfo?.user) {
        try {
          const user = session._userInfo.user;
          messageRouter.registerUser(user.id, clientId, user.displayName || user.email, user.role === 'admin');
          messageRouter.updatePresence(user.id, true);
          console.log(`[Scratchy] User registered with message router: ${user.email} (${user.id})`);
        } catch(e) { console.error('[Scratchy] Message router registration failed:', e.message); }
      }
    ...
    processGenUIIfNeeded(frame, (enhancedFrame) => {
      ...
      if (targetWs && targetWs.readyState === 1) {
        ...
        // Distinguish between agent response loss (critical) vs lifecycle event loss (noise)
        const isChatFinal = frame.event === 'chat' && frame.payload?.state === 'final';
        const isAssistantDelta = frame.event === 'chat' && frame.payload?.state === 'delta';
        if (isChatFinal) {
          console.log(`[Scratchy] 🔴 Agent RESPONSE lost (no client WS) — seq ${session.seq}, buffered for replay`);
        } else if (!isAssistantDelta) {
...
function _wireClientToGateway(clientWs, session) {
  ...
  clientWs.on("message", (data) => {
    ...
    // On reconnect/takeover, handshakeComplete is already true — drop stale connect frames
    // to avoid "connect is only valid as the first request" errors from the gateway.
    if (session.handshakeComplete && raw.includes('"method":"connect"')) {
      try {
        const _f = JSON.parse(raw);
        if (_f.type === "req" && _f.method === "connect") {
          console.log(`[Scratchy] Dropping stale connect frame on reconnected session (handshake already complete)`);
          return;
        }
      } catch(e) {}
    }
    ...
    // Phase 32: Handle direct-message (user-to-user messaging)
    if (raw.includes('"type":"direct-message-send"')) {
      try {
        const frame = JSON.parse(raw);
        if (frame.type === 'direct-message-send' && messageRouter) {
          const fromUser = session._userInfo?.user?.id || 'legacy';
          const toUser = frame.recipientId;
          ...
          if (!toUser || !content) {
            ...
            const errorFrame = {
              ...
              payload: { state: 'final', message: '⚠️ Missing recipient or content' }
            };
          ...
          const result = messageRouter.sendMessage(fromUser, toUser, content);

          // Confirm to sender
          session.seq++;
          const confirmFrame = {
            type: 'event', event: 'direct-message-confirm',
            payload: { messageId: result.id, status: result.status }
          };
          try { session.clientWs.send(JSON.stringify({ seq: session.seq, frame: confirmFrame })); } catch(e) {}

          console.log(`[Scratchy] Direct message from ${fromUser} to ${toUser}: ${result.status}`);
        }
      } catch(e) {
        console.error('[Scratchy] Error handling direct-message-send:', e.message);
      }
      return;
    }

    // Phase 32: Handle direct-message-list (load conversation history)
    if (raw.includes('"type":"direct-message-list"')) {
      try {
        const frame = JSON.parse(raw);
        if (frame.type === 'direct-message-list' && messageRouter) {
          const userId = session._userInfo?.user?.id || 'legacy';
          const otherUser = frame.userId;
          ...
          if (!otherUser) {
            ...
            session.seq++;
            const responseFrame = {
              type: 'event', event: 'direct-message-conversations',
              payload: { conversations }
            };
            try { session.clientWs.send(JSON.stringify({ seq: session.seq, frame: responseFrame })); } catch(e) {}
          } else {
            // Load messages for specific conversation
            const convId = messageRouter._getConversationId(userId, otherUser);
            const messages = messageRouter.getMessages(convId, frame.beforeId, frame.limit || 50);
            session.seq++;
            const responseFrame = {
              type: 'event', event: 'direct-message-history',
              payload: { conversationId: convId, messages, otherUser }
            };
            try { session.clientWs.send(JSON.stringify({ seq: session.seq, frame: responseFrame })); } catch(e) {}
          }
        }
      } catch(e) {
        console.error('[Scratchy] Error handling direct-message-list:', e.message);
      }
      return;
    }

    // Phase 32: Handle direct-message-users (list available users)
    if (raw.includes('"type":"direct-message-users"')) {
      try {
        const frame = JSON.parse(raw);
        if (frame.type === 'direct-message-users' && messageRouter) {
          const userId = session._userInfo?.user?.id || 'legacy';
          const user = session._userInfo?.user;
          const isAdmin = user?.role === 'admin';
          ...
          session.seq++;
          const responseFrame = {
            type: 'event', event: 'direct-message-users-list',
            payload: { users }
          };
          try { session.clientWs.send(JSON.stringify({ seq: session.seq, frame: responseFrame })); } catch(e) {}
        }
      } catch(e) {
        console.error('[Scratchy] Error handling direct-message-users:', e.message);
      }
      return;
    }

    // Phase 32: Handle direct-message-read (mark conversation as read)
    if (raw.includes('"type":"direct-message-read"')) {
      try {
        const frame = JSON.parse(raw);
        if (frame.type === 'direct-message-read' && messageRouter) {
          const userId = session._userInfo?.user?.id || 'legacy';
          const convId = frame.conversationId;
          if (convId) {
            messageRouter.markRead(convId, userId);
          }
        }
      } catch(e) {
        console.error('[Scratchy] Error handling direct-message-read:', e.message);
      }
      return;
    }
      ...
      try {
        ...
        if (frame.type === "widget-action") {
          ...
          const _sendOps = (result, _userInitiated = true) => {
            if (result && result.ops && result.ops.length > 0) {
              console.log('[Scratchy] 📤 _sendOps sending', result.ops.length, 'ops');
              ...
              try {
                ...
                if (ws && ws.readyState === 1) {
                  ...
                  console.log('[Scratchy] 📤 _sendOps sent successfully');
                } else {
                  console.log('[Scratchy] ⚠️ _sendOps WS not open, state:', ws ? ws.readyState : 'null');
                }
          ...
          if (action.startsWith('sn-')) {
            ...
            session._adminWidget.handleAction(action, context)
              .then((result) => {
                ...
                // DEBUG: Log first few ops to verify content
                if (result && result.ops && result.ops.length > 0) {
                  console.log('[Scratchy] 📦 Admin first op:', JSON.stringify(result.ops[0]));
                  console.log('[Scratchy] 📦 Admin last op id:', result.ops[result.ops.length - 1].id);
                }
...
function _cleanupSession(clientId, reason) {
  ...
  // Phase 32: Update message router presence when user disconnects
  if (messageRouter && session._userInfo?.user) {
    try {
      const user = session._userInfo.user;
      messageRouter.updatePresence(user.id, false);
      console.log(`[Scratchy] User disconnected from message router: ${user.email} (${user.id})`);
    } catch(e) { console.error('[Scratchy] Message router disconnect failed:', e.message); }
  }
...
server.listen({ port: PORT, host: "::", exclusive: false }, () => {
  ...
  // Phase 32: Initialize user-to-user messaging
  messageRouter = new MessageRouter();
  messageRouter.init();
  messageRouter.attach(wss);
  console.log(`[Scratchy] User-to-user messaging initialized`);

  // Start systemd watchdog after server is listening
  startWatchdog();
});
