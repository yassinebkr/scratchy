  ...
  for (const file of files) {
    ...
    try {
      ...
      if (tpl && tpl.id) {
        ...
        console.error(`[widget-templates] ✅ Loaded template: ${tpl.id} (${tpl.name})`);
      }
    } catch (err) {
      console.error(`[widget-templates] ⚠️ Failed to load ${file}:`, err.message);
    }
  }
  _loaded = true;
  console.error(`[widget-templates] ${templates.size} templates ready`);
}
