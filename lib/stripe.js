...
/* ------------------------------------------------------------------ */
/*  Config — reads from secure-keys (DB) with .env fallback           */
/* ------------------------------------------------------------------ */
...
/** Late-bind secure key store (called after DB init) */
export function setSecureKeys(secureKeys) { _sk = secureKeys; }

/** Get a config value: secure store → env fallback */
function _key(name) {
  try { return _sk?.getKey(name); } catch { /* not ready */ }
  return process.env[name];
}

function getStripeKey() {
  const key = _key('STRIPE_SECRET_KEY');
  if (!key) throw new Error('STRIPE_SECRET_KEY not set');
  return key;
}

function getWebhookSecret() {
  return _key('STRIPE_WEBHOOK_SECRET') || '';
}

/** Map Stripe price IDs → plan names */
function getPriceMap() {
  return {
    [_key('STRIPE_PRICE_PRO') || 'price_pro']: 'pro',
    [_key('STRIPE_PRICE_TEAM') || 'price_team']: 'max',
  };
}

/** Map plan names → Stripe price IDs */
function getPlanPriceId(plan) {
  if (plan === 'pro') return _key('STRIPE_PRICE_PRO') || '';
  if (plan === 'max' || plan === 'team') return _key('STRIPE_PRICE_TEAM') || '';
  return '';
}
