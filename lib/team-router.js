  ...
  return {
    ...
    async executeTool(name, args) {
      ...
      try {
        ...
        // AbortController so timeout actually cancels the underlying fetch/SSE
        const delegationAbort = new AbortController();
        const delegationTimer = setTimeout(() => {
        ...
        const routePromise = adapter.routeMessageStreaming(
          ...
          delegationAbort.signal  // pass abort signal to adapter
        );

        try {
          workerResponse = await routePromise;
        } finally {
