    ...
    if (looksLikeWork && workers.length >= 2) {
      ...
      try {
        ...
        if (error) {
          ...
          // ── Preview link for generated files ──
          // Files were already published immediately after each worker completed.
          // Just check which batches have HTML and send preview buttons.
          const previewOps = [];
          for (const result of taskResults) {
            if (!result.hasHtmlFiles || !result.taskBatchId) continue;
            previewOps.push({
              op: 'upsert',
              id: `preview-${result.taskBatchId}`,
              type: 'buttons',
              data: {
                title: '🖥️ Preview generated files',
                buttons: [
                  { label: '👁️ Open Preview', action: `open-preview:/api/preview/${userId}/${result.taskBatchId}/index.html`, style: 'primary' },
                ],
              },
            });
          }
          if (previewOps.length > 0) {
            sendJson(ws, { type: 'canvas-ops', ops: previewOps, source: 'team-preview', ts: Date.now() });
            console.log(`[team-router] Sent ${previewOps.length} preview button(s)`);
          }
      ...
      try {
        ...
        try {
          batchDirs = readdirSync(nullclawWsRoot).filter(d => {
            ...
            const s = statS(p);
            // Only include dirs created during this team run (skip stale batches)
            ...
            if (files.some(f => f.rel.endsWith('.html'))) {
              ...
              legacyPreviewOps.push({
                ...
                data: {
                  title: '🖥️ Preview generated files',
                  buttons: [
                    { label: '👁️ Open Preview', action: `open-preview:/api/preview/${userId}/${batchId}/index.html`, style: 'primary' },
                  ],
                },
              });
            }
        ...
        if (legacyPreviewOps.length > 0) {
          sendJson(ws, { type: 'canvas-ops', ops: legacyPreviewOps, source: 'team-preview', ts: Date.now() });
          console.log(`[team-router] Legacy loop: sent ${legacyPreviewOps.length} preview button(s)`);
        }
      } catch (err) {
