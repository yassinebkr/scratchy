 ...
 */
async function publishTaskFiles(userId, taskBatchId, publishName = null) {
  ...
  const files = await scanDir(sandboxDir);
  if (files.length === 0) {
  ...
  await cp(sandboxDir, publishedDir, { recursive: true });
    ...
    async executeTool(name, args) {
      ...
      try {
        ...
        try {
          ...
          let files = await scanDir(nullclawWorkerWs).catch(() => []);
          if (files.length === 0) {
            // Fallback: check task sandbox in case future NullClaw versions respect workspaceDir
            files = await scanDir(taskDir).catch(() => []);
          }
          if (files.length > 0) {
            const listing = files.map(f => `  ${f.rel} (${f.size})`).join('\n');
            fileManifest = `\n\n[Files Created — sandbox: ${taskBatchId}]\n${listing}\n[/Files]`;
            console.log(`[team-router] Worker ${agentName}: found ${files.length} files in sandbox ${taskBatchId}`);
          }
...
function buildWorkerPrompt(agentInfo, task, context, team, fileContents = '', userId = null, taskBatchId = null) {
  ...
  if (taskBatchId) {
    parts.push(`When your task involves creating files (websites, apps, configs, etc.), use the file_write tool to create actual files.`);
    parts.push(`⚠️ CRITICAL: Prefix ALL file paths with "${taskBatchId}/". Example: file_write path="${taskBatchId}/index.html". This puts files in your isolated sandbox. Files written without this prefix will be lost.`);
  } else {
...
export async function routeTeamMessage(opts) {
  ...
  try {
    ...
    for (let turn = 0; turn < MAX_DELEGATION_TURNS; turn++) {
      ...
      if (orchestratorUsedFileWrite && !enforcementUsed) {
