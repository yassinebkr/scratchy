...
export async function routeTeamMessage(opts) {
  ...
  try {
    ...
    try {
      ...
      console.log(`[team-router] Preview scan: publishedRoot=${publishedRoot} teamStartTime=${teamStartTime} (${new Date(teamStartTime).toISOString()})`);
      let pubDirs = [];
      try { pubDirs = readdirS(publishedRoot); } catch (e) {
        console.log(`[team-router] Preview scan: cannot read publishedRoot: ${e.message}`);
      }
      console.log(`[team-router] Preview scan: found ${pubDirs.length} dirs: ${pubDirs.join(', ')}`);
      for (const name of pubDirs) {
        const dir = join(publishedRoot, name);
        try {
          const s = statS2(dir);
          if (!s.isDirectory()) { console.log(`[team-router] Preview scan: ${name} is not a directory, skipping`); continue; }
          if (s.mtimeMs < teamStartTime) { console.log(`[team-router] Preview scan: ${name} too old (mtime=${new Date(s.mtimeMs).toISOString()} < start=${new Date(teamStartTime).toISOString()})`); continue; }
          const files = await scanDir(dir);
          const hasHtml = files.some(f => f.rel.endsWith('.html'));
          console.log(`[team-router] Preview scan: ${name} — ${files.length} files, hasHtml=${hasHtml}`);
          if (hasHtml) {
            endPreviewUrls.push(`/api/preview/${userId}/${name}/index.html`);
          }
        } catch (e) { console.log(`[team-router] Preview scan: error for ${name}: ${e.message}`); }
      }
      console.log(`[team-router] Preview scan result: ${endPreviewUrls.length} URLs: ${JSON.stringify(endPreviewUrls)}`);
    } catch (e) { console.error(`[team-router] Preview scan failed:`, e.message); }
