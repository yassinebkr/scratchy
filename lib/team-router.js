...
export async function routeTeamMessage(opts) {
  ...
  try {
    ...
    for (let turn = 0; turn < MAX_DELEGATION_TURNS; turn++) {
      response = '';
      // Track delegations intercepted from http_request tool calls
      const httpDelegations = [];
      ...
      response = await adapter.routeMessageStreaming(
        ...
        // onToolEvent — intercept http_request delegation calls + forward others
        (evt) => {
          // Intercept http_request calls to delegation endpoint
          // (NullClaw's LLM may use http_request instead of [DELEGATE] blocks)
          if (evt.type === 'tool_call_start' && evt.name === 'http_request') {
            const args = safeParseJson(evt.arguments);
            const url = args?.url || args?.uri || '';
            if (url.includes('team-delegate')) {
              // Convert http_request delegation to our tracked format
              const body = safeParseJson(args?.body);
              if (body?.agentId && body?.task) {
                httpDelegations.push({
                  agentId: body.agentId,
                  task: body.task,
                  context: body.context || '',
                });
                console.log(`[team-router] Intercepted http_request delegation → ${body.agentId}: ${body.task.slice(0, 60)}`);
              }
              // Show as delegation in UI, not raw http_request
              sendJson(ws, {
                type: 'tool_call',
                tool: 'delegate',
                args: { agentId: body?.agentId, task: body?.task },
                agentId: orchestratorAgentId,
                teamId,
                ts: Date.now(),
              });
          ...
          if (evt.type === 'tool_call_start') {
            ...
            // Skip http_request results for delegation calls (we handle them ourselves)
            if (evt.name === 'http_request') {
              const output = evt.output || '';
              if (output.includes('team-delegate') || output.includes('DEPRECATED')) {
                return; // suppress deprecated endpoint results
              }
            }
            sendJson(ws, {
              type: 'tool_result',
      ...
      // ── Parse delegation blocks from response + merge http_request intercepts ──
      const { delegations: parsedDelegations, cleanText } = parseDelegationBlocks(response);
      const delegations = [...parsedDelegations, ...httpDelegations];

      console.log(`[team-router] Turn ${turn}: response length=${response.length}, parsed=${parsedDelegations.length}, http_intercept=${httpDelegations.length}, total=${delegations.length}`);
