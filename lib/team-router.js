...
import { createDirectProviderFromConfig } from './direct-provider.js';

/* ------------------------------------------------------------------ */
/*  Direct provider singleton (Sonnet for workers — fast + parallel)  */
/* ------------------------------------------------------------------ */
let _directProvider = null;

function getDirectProvider() {
  if (!_directProvider) {
    _directProvider = createDirectProviderFromConfig({ model: 'claude-sonnet-4-20250514' });
    if (_directProvider) {
      console.log(`[team-router] Direct provider initialized: ${_directProvider.model}`);
    } else {
      console.warn('[team-router] Direct provider unavailable — falling back to NullClaw for workers');
    }
  }
  ...
  return {
    ...
    async executeTool(name, args) {
      ...
      try {
        ...
        try {
          // ── Strategy 1: Direct Anthropic API (fast, parallel, Sonnet) ──
          const directProvider = getDirectProvider();
          if (directProvider) {
            const startMs = Date.now();
            console.log(`[team-router] Worker ${agentName}: using direct API (${directProvider.model})`);

            // Worker's system prompt + team context
            const systemPrompt = [
              agentInfo.systemPrompt || '',
              `You are part of team "${team.name}". Provide a clear, complete response to your assigned task.`,
              'Produce your output as text in the response. Include code in fenced code blocks.',
              'Do NOT attempt to write files or use tools — just output the content directly.',
            ].filter(Boolean).join('\n\n');

            workerResponse = await directProvider.chat(
              systemPrompt,
              workerPrompt,
              onChunk,
            ...
            console.log(`[team-router] Worker ${agentName}: direct API completed in ${Math.floor((Date.now() - startMs) / 1000)}s (${workerResponse.length} chars)`);
          } else {
            // ── Strategy 2: NullClaw fallback (sequential, Opus) ──
            console.log(`[team-router] Worker ${agentName}: using NullClaw fallback`);
...
export async function routeTeamMessage(opts) {
  ...
  try {
    ...
    for (let turn = 0; turn < MAX_DELEGATION_TURNS; turn++) {
      ...
      // NullClaw is single-threaded — too many delegations = sequential queue.
      // Cap at MAX_CONCURRENT_DELEGATIONS per turn; orchestrator can delegate more in next turn.
