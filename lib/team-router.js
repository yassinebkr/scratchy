...
const TEAM_ROUTING_TIMEOUT_MS = 1_800_000; // 30 min hard cap — enough for 3 turns of 3 sequential workers
...
function buildTeamSystemPrompt(team, orchestratorAgent, teamMemory, userId) {
  ...
  parts.push('');
  parts.push('Multiple blocks run sequentially. After results come back, delegate more or synthesize.');
  parts.push('');
  parts.push('Rules:');
  ...
  return {
    ...
    async executeTool(name, args) {
      ...
      try {
        ...
        const workerPrompt = buildWorkerPrompt(agentInfo, task, context, team, fileContents);
        let workerResponse = '';
 ...
 * @returns {string}
 */
function buildWorkerPrompt(agentInfo, task, context, team, fileContents = '') {
  const parts = [];
