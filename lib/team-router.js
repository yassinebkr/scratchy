 ...
 * @returns {string}
 */
function buildTeamSystemPrompt(team, orchestratorAgent, teamMemory) {
  const parts = [];
  ...
  parts.push('Use the `delegate` tool to assign subtasks to team agents.');
  parts.push('Parameters:');
  parts.push('  - agentId: the agent\'s ID from the roster above');
  parts.push('  - task: clear description of what the agent should do');
  parts.push('  - context: (optional) additional context the agent needs');
  parts.push('');
  parts.push('Guidelines:');
  parts.push('- Break complex tasks into subtasks for specialized agents');
  ...
  parts.push('- After receiving results, synthesize a coherent final answer');
  parts.push('- For simple tasks, you can answer directly without delegating');
...
export async function routeTeamMessage(opts) {
  ...
  const teamSystemPrompt = buildTeamSystemPrompt(team, orchestratorAssignment, teamMemory);
  ...
  try {
    ...
    if (response && parseGenUIResponse) {
      ...
      if (hasOps && ops.length > 0) {
        // Clear stale canvas (dashboard, previous team responses) before rendering new tiles
        sendJson(ws, { type: 'canvas-ops', ops: [{ op: 'clear' }], source: 'team-genui-clear', teamId, ts: Date.now() });
        ...
        // Persist canvas state (start fresh — clear applied above)
        if (canvasState) {
          try {
            const merged = [];
            for (const op of ops) {
