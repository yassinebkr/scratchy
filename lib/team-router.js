  ...
  try {
    ...
    for (let turn = 0; turn < MAX_DELEGATION_TURNS; turn++) {
      ...
      if (turn === 0 && !batchEnforcementUsed && expectedTasks >= 2 && delegations.length < expectedTasks) {
        ...
        console.log(`[team-router] Batch enforcement: ${delegations.length} delegation(s) but ${expectedTasks} tasks detected — re-prompting`);
        ...
        continue; // retry Turn 0 with all tasks
      }
