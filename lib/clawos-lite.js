/**
 * @fileoverview ClawOS-Lite — Lightweight injection detection for Scratchy v2.
 *
 ...
 *
 * @module lib/clawos-lite
 */

// ─── External Injection Patterns (ported from ClawOS index.ts) ──────────────
...
export function shouldBlockTool(state, toolName) {
  ...
  if (DANGEROUS_TOOLS.has(toolName)) {
    return {
      blocked: true,
      reason: `[ClawOS-Lite] Tool "${toolName}" blocked — ${state.signalCount} injection signal(s) ` +
