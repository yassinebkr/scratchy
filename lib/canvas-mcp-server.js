...
const TOOLS = [
...

  {
    ...
    description: 'Define AND render a standalone UI component (Web Component + Shadow DOM). The widget runs as a SELF-CONTAINED APP — all interactivity (drag-and-drop, sorting, filtering, toggling) MUST be handled client-side via the "js" field. The agent only provides initial data; the widget operates independently after creation. To update widget data later, use the update tool with id "lw-{widgetId}".',
    inputSchema: {
      type: 'object',
      properties: {
        widgetId: {
