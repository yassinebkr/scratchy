...
// Load widget templates on startup
widgetTemplates.loadAll().catch(err => {
  console.warn('[canvas-mcp] Failed to load widget templates:', err.message);
});
...
const TOOLS = [
...

  {
    ...
    description: 'PREFERRED over create_live_widget. Create an interactive widget from a pre-built template. Instant rendering (<100ms) — no need to generate HTML/CSS/JS. Available templates: kanban (drag-and-drop board), timer (pomodoro countdown), expense-tracker (add/delete items with totals), poll (voting with results), quiz (multi-step with scoring), counter (increment/decrement), checklist (toggle/add/delete with progress). Use this whenever a template matches the request. Fall back to create_live_widget only for truly custom widgets.',
    inputSchema: {
      type: 'object',
      properties: {
        template: {
          type: 'string',
          description: 'Template ID: kanban | timer | expense-tracker | poll | quiz | counter | checklist',
          enum: ['kanban', 'timer', 'expense-tracker', 'poll', 'quiz', 'counter', 'checklist'],
        },
        widgetId: {
          type: 'string',
          description: 'Unique widget instance ID (lowercase, hyphens ok)',
        },
        config: {
          type: 'object',
          description: 'Template-specific configuration. Each template has its own schema — provide the data the user requested. Examples: kanban needs {title, columns, cards}, timer needs {workMinutes, breakMinutes, label}, expense-tracker needs {title, currency, categories}, poll needs {question, options}, quiz needs {title, questions}, counter needs {label, value, min, max, step}, checklist needs {title, items}.',
        },
      },
      required: ['template', 'widgetId'],
    },
  },

  {
    name: 'create_live_widget',
    description: 'Define AND render a CUSTOM standalone UI component (Web Component + Shadow DOM). Use ONLY when no template matches (check create_from_template first). The widget runs as a SELF-CONTAINED APP — all interactivity MUST be handled client-side via the "js" field. Art direction: dark theme (bg #0d0b07, surface #1a1610, text #f0ead6, accent #F9A602), compact padding (8-10px), subtle transitions (0.2s ease), 8px border-radius. Cards should be tight — title + badge on same row, description max 2 lines.',
    inputSchema: {
      type: 'object',
      properties: {
        widgetId: {
          type: 'string',
          description: 'Unique type name (lowercase, hyphens only, 2-49 chars). Instance id on canvas: "lw-{widgetId}".',
          pattern: '^[a-z][a-z0-9-]{1,48}$',
        },
        html: {
          type: 'string',
          description: 'HTML template with mustache bindings: {{prop}} (escaped), {{{raw}}} (unescaped), {{#each arr}}...{{/each}}, {{#if prop}}...{{/if}}. Use data-action="name" on interactive elements. For drag-and-drop: data-action="dragstart" on draggable items (with data-card-id or data-item-id), data-action="drop" on drop zones (with data-column or data-target). NEVER use inline event handlers (onclick, ondragover, etc.) — they are stripped.',
        },
        css: {
          type: 'string',
          description: 'Scoped CSS (Shadow DOM, no leaks). Dark theme: var(--bg, #0d0b07), var(--surface, #1a1610), var(--text, #f0ead6), var(--text-muted, #8a7e6a), var(--accent, #F9A602), var(--border, rgba(255,255,255,0.06)), var(--radius, 8px). Add .dragging{opacity:0.4} and .drag-over{background:rgba(249,166,2,0.1)} classes for DnD visual feedback.',
        },
        props: {
          type: 'array',
          items: { type: 'string' },
          description: 'Reactive property names. Re-renders on patch/update.',
        },
        actions: {
          type: 'array',
          items: {
            type: 'object',
            properties: {
              name: { type: 'string', description: 'Action name (matches data-action in HTML)' },
              emits: { type: 'string', description: 'Event name emitted. Only reaches agent if js handler returns false/undefined.' },
            },
            required: ['name'],
          },
          description: 'Action declarations. If a js handler exists and returns true, the action stays local. Otherwise it propagates to the agent.',
        },
        js: {
          type: 'string',
          description: 'Client-side action handler (function body, called per action). Params: action (string — the emits name), payload (object: all data-* attrs from the element + dragId for DnD), data (mutable widget state — modify in-place), render (call after data changes to re-render), root (shadow DOM root). Return true = handled locally (NO agent call). Return false/undefined = propagate to agent. CRITICAL: This function is called fresh per action — do NOT use local variables to pass state between calls (e.g. let dragId). Use payload.dragId (set automatically by the DnD system from data-card-id/data-item-id) or store temp state on data (e.g. data._temp). Always return true for UI-only actions. Example kanban DnD: if(action==="card-drop"){const id=payload.dragId;const cols=["todo","doing","done"];let card;for(const c of cols){const i=(data[c]||[]).findIndex(x=>x.id===id);if(i>=0){card=data[c].splice(i,1)[0];break;}}if(card&&payload.column){data[payload.column].push(card);render();}return true;}return true;',
        },
        defaults: {
          type: 'object',
          description: 'Default/initial values for props.',
        },
      },
...
function handleCreateLiveWidget(args) {
  const { widgetId, html, css, props, actions, defaults, js } = args;
  ...
  // Client-side action handler — runs locally, no LLM round-trip
    ...
    case 'tools/call': {
      ...
      switch (toolName) {
        ...
        case 'create_from_template': {
          ...
          if (tplResult.error) {
            result = { ops: [], message: `❌ ${tplResult.error}` };
          } else {
            result = {
              ops: tplResult.ops,
              message: `✅ Widget "${widgetId}" created from template "${template}" (instant render).`,
            };
          }
          break;
        }
