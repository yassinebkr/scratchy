...

  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        js: {
          type: 'string',
          description: 'Client-side action handler (runs as function body). Parameters: action (string), payload (object with data-* attrs + dragId for DnD), data (mutable widget state — modify directly), render (call after changing data to re-render), root (shadow DOM root). Return true to handle locally (no agent round-trip). Return false/undefined to propagate to agent. Example for kanban drag-drop: if(action==="card-drop"){const lists=["todo","doing","done"];let card;for(const l of lists){const i=(data[l]||[]).findIndex(c=>c.id===payload.dragId);if(i>=0){card=data[l].splice(i,1)[0];break;}}if(card){(data[payload.column]||(data[payload.column]=[])).push(card);render();}return true;}',
        },
        defaults: {
          type: 'object',
