...
/**
 * @typedef {'user'|'user-selfhosted'|'orchestrator'|'worker'|'reviewer'} AgentRole
 */
...
const ROLE_AUTONOMY = {
  ...
  /** Orchestrator — memory-only, no execution */
  orchestrator: {
    level: 'locked',
    workspace_only: true,
    block_high_risk_commands: true,
 ...
 */
const ROLE_MODELS = {
  user: 'anthropic/claude-opus-4-20250514',
  'user-selfhosted': 'anthropic/claude-opus-4-20250514',
...
export function generateConfig(userId, role, opts = {}) {
  const homeDir = path.join(os.tmpdir(), `nullclaw-${userId}-${role}`);
...
export function cleanupStaleConfigs() {
  ...
  try {
    ...
    if (cleaned > 0) {
      console.log(`[tool-policy] Cleaned up ${cleaned} stale NullClaw config dir(s)`);
    }
