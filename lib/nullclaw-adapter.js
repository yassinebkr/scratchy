...
export class NullClawAdapter extends EventEmitter {
  ...
  async routeMessageStreaming(userId, message, onChunk, sessionKey, onToolEvent, externalSignal) {
    // ── Per-instance request lock ──
    // NullClaw is single-threaded: concurrent requests to the same instance
    // cause queueing, timeouts, and destroy/respawn race conditions.
    // Serialize requests per instance key so each gets a clean shot.
    while (this.#requestLocks.has(userId)) {
      this.emit('log', { userId, stream: 'system', line: `Waiting for lock on instance ${userId}...` });
      try {
        await this.#requestLocks.get(userId);
      } catch { /* previous request failed — lock released, proceed */ }
    }

    // If external caller already aborted while waiting for lock, bail
    if (externalSignal?.aborted) throw new DOMException('Aborted by caller', 'AbortError');

    // Set lock (resolved/rejected when this request completes)
    let releaseLock;
    const lockPromise = new Promise((resolve, reject) => { releaseLock = { resolve, reject }; });
    this.#requestLocks.set(userId, lockPromise);

    try {
      return await this.#routeWithRetries(userId, message, onChunk, sessionKey, onToolEvent, externalSignal);
    } finally {
      this.#requestLocks.delete(userId);
      releaseLock.resolve();
    }
  }
