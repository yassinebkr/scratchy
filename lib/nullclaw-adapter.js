...
export class NullClawAdapter extends EventEmitter {
  ...
  /** @type {Map<string, Promise<void>>} per-instance request lock — prevents concurrent requests */
  // #requestLocks removed — NullClaw multi-threaded fork handles concurrency natively
   ...
   */
  #allocatePort() {
    // NullClaw threaded fork uses port AND port+1 (health thread).
    // Allocate in steps of 2 to avoid collisions.
    for (let port = this.#portMin; port <= this.#portMax; port += 2) {
      if (!this.#usedPorts.has(port) && !this.#usedPorts.has(port + 1)) {
        this.#usedPorts.add(port);
        this.#usedPorts.add(port + 1); // reserve health port
        return port;
      }
    }
    throw new Error(
  ...
  #releasePort(port) {
    ...
    this.#usedPorts.delete(port + 1); // release health port too
  }
   ...
   * @param {string} userId
   * @param {Object} [opts]
   * @param {string} [opts.role] — agent role (user/orchestrator/worker/reviewer)
   * @param {string} [opts.homeDir] — custom HOME dir (for role-specific NullClaw config)
   * @returns {Promise<InstanceState>}
   */
  async spawnInstance(userId, opts = {}) {
    // Instance key: userId for user role, userId:role for team roles
    const key = opts.role && opts.role !== 'user' && opts.role !== 'user-selfhosted'
      ? `${userId}:${opts.role}` : userId;
    ...
    const instance = {
      ...
      healthFailures: 0,
      idleTimer: null,
      _role: opts.role || 'user',
      _homeDir: opts.homeDir || null,
    };
    ...
    try {
      ...
      this.emit('error', { userId: key, error: err });
      throw err;
    }

    this.#resetIdleTimer(key);
    this.emit('spawn', { userId: key, port, pid: instance.pid, role: opts.role || 'user' });
  ...
  async #startProcess(instance) {
    ...
    // Build env — override HOME for role-specific configs
    const env = { ...process.env, ...this.#env, PORT: String(instance.port) };
    if (instance._homeDir) {
      env.HOME = instance._homeDir;
      this.emit('log', {
        userId: instance.userId,
        stream: 'system',
        line: `Using custom HOME=${instance._homeDir} (role=${instance._role})`,
      });
    }

    // Low scheduler priority — other services always get CPU first.
    // Service-level CPUQuota=25% (systemd unit) caps the total.
    const child = spawn('nice', ['-n', '19', this.#command, ...args], {
  ...
  async #runHealthChecks() {
    for (const [userId, instance] of this.#instances) {
      ...
      // NullClaw multi-threaded fork: health endpoint on port+1 always responds
      // (dedicated thread), so no active grace period needed.
      try {
  ...
  async #pingHealth(port) {
    // NullClaw multi-threaded fork: health endpoint runs on port+1
    // in a dedicated thread, so it always responds even during active SSE streams.
    ...
    try {
      ...
      // Fallback: try main port (for non-threaded NullClaw instances)
      try {
        const controller2 = new AbortController();
        const timeout2 = setTimeout(() => controller2.abort(), 2_000);
  ...
  async routeMessageStreaming(userId, message, onChunk, sessionKey, onToolEvent, externalSignal) {
    ...
    // no per-instance request lock needed. Each request gets its own worker
    // thread in the gateway, and SessionManager uses RwLock for concurrent
    // session access. Different session_keys run in true parallel.
    if (externalSignal?.aborted) throw new DOMException('Aborted by caller', 'AbortError');
    return await this.#routeWithRetries(userId, message, onChunk, sessionKey, onToolEvent, externalSignal);
  }

  /**
   * Internal retry loop — called under the per-instance lock.
   * @private
   */
  async #routeWithRetries(userId, message, onChunk, sessionKey, onToolEvent, externalSignal) {
    let lastErr;
  ...
  async #doRouteMessageStreaming(userId, message, onChunk, sessionKey, onToolEvent, externalSignal) {
    ...
    // Pre-routing health check — verify instance is alive before sending request.
    // NullClaw multi-threaded fork: health endpoint on port+1 always responds
    // (separate thread), so no active grace period hack needed.
    const healthy = await this.#pingHealth(instance.port);
    if (!healthy) {
      this.emit('log', {
        userId,
        stream: 'system',
        line: `Pre-routing health check failed for port ${instance.port} — destroying and respawning`,
      });
      this.destroyInstance(userId);
      instance = await this.spawnInstance(userId);
      await this.#waitForReady(instance, 10_000);
    ...
    try {
      ...
      console.log(`[adapter:stream] POST http://127.0.0.1:${instance.port}/api/message — session=${sessionKey ?? `api:${userId}`} body=${msgBody.length}B`);
      ...
      console.log(`[adapter:stream] Response ${res.status} in ${Date.now() - fetchStart}ms`);
      ...
      while (true) {
        ...
        // Signal that the connection is alive (for inactivity timer reset).
        // This fires even for SSE comments like :keepalive from NullClaw.
        if (onChunk && value?.length) onChunk('');
        ...
        while ((eventEnd = buffer.indexOf('\n\n')) !== -1) {
          ...
          if (eventType === 'tool_call_start' || eventType === 'tool_call_result' || eventType === 'iteration_start') {
            console.log(`[adapter:stream] SSE event: ${eventType} — ${data.slice(0, 200)}`);
            if (onToolEvent) {
              try {
                const parsed = JSON.parse(data);
                onToolEvent({ type: eventType, ...parsed });
              } catch (e) {
                console.error(`[adapter:stream] Failed to parse ${eventType}: ${e.message}`);
              }
            }
            continue;
          }
          ...
          try {
            ...
            if (parsed.delta) {
              ...
              if (fullText.length <= 100 || fullText.length % 500 === 0) {
                console.log(`[adapter:stream] delta chunk, fullText=${fullText.length}B`);
              }
            }
          } catch {
  ...
  /**
   * Ensure an instance is healthy and ready to receive requests.
   * If the instance is dead or unresponsive, destroy and respawn it.
   * Call this before sending to an instance that may have been idle.
   *
   * @param {string} userId — instance key
   * @returns {Promise<void>}
   */
  async ensureReady(userId) {
    const instance = this.#instances.get(userId);
    if (!instance || instance.status === 'stopped' || instance.status === 'error') {
      this.emit('log', { userId, stream: 'system', line: `ensureReady: instance ${instance?.status ?? 'missing'} — spawning` });
      await this.spawnInstance(userId);
      return;
    }
    if (instance.status === 'starting') {
      await this.#waitForReady(instance, 10_000);
    ...
    // Instance exists and is 'running' — ping health
    const ok = await this.#pingHealth(instance.port);
    if (!ok) {
      this.emit('log', { userId, stream: 'system', line: `ensureReady: health check failed on port ${instance.port} — destroying and respawning` });
      this.destroyInstance(userId);
      await this.spawnInstance(userId);
    }
  }
